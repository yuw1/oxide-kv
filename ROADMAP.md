...
| P8 | Pre-vote, joint consensus, tx timeout, metrics, prod cluster test | ✅ Merged | PR #33–#43 (+ hotfixes #50/#51) | Disruptive-server problem closed; ops hardening done; all 6 acceptance criteria verified on master |
...
1. **Virtual clock + virtual network.** Replace wall-clock timers and
   ...
   (`rust/oxide-kv/src/raft/transport.rs`) and the clock behind traits, with a
...
| ✅ #6a | `feat(raft): JoinCluster RPC for cold-new-server catch-up` | `JoinClusterRequest`/`JoinClusterResponse` wire messages (proto tags 9/10) + `RaftNode::handle_join_cluster` (leader-only). Candidate sends `JoinCluster(candidate_addr)` to a hint address; leader replies with the current peer list; candidate calls `set_peers` and waits to be added via Joint consensus. Bugfix: `next_index` default for unknown peer changed from `log_len + 1` to `1` so cold-new-server catch-up works. 7 new unit tests + 4 new integration tests in `tests/join_cluster.rs`. |
| ✅ #7 | `feat(raft): tx timeout + admin-driven abort` | Coordinator-side timeout sweep (`run_tx_timeout_loop` spawned in `main.rs`, `OXIDE_TX_TIMEOUT_MS` default 30 s, sweep interval default 1 s) + admin `AbortTx` JSON RPC. Both translate to a `DecideTx(Abort)` log entry which replicates through the normal AppendEntries path; followers apply via existing `apply_logs` and purge `pending_txs`. New `PendingTx::begin_unix_ms` field with `serde(default)` for forward-compat. Wire: `Command.Body::abort_tx = 11`. 6 lib tests (`raft/node.rs` + `raft/coordinator.rs`) + 4 state-machine tests + 3 integration tests in `tests/tx_timeout_abort.rs` = **249 → 255 lib tests, +3 integration tests.** Priority-B recovery (participant-side autonomous abort) deferred. Merged via PR #40 (squash `d2e53f7`). |
| ✅ #8 (merged via PR #42) | `feat(observability): Prometheus textfile exporter + OpenTelemetry traces` | Native HTTP `/metrics` endpoint (hand-rolled minimal HTTP/1.1, no `hyper` dep) on `--metrics-addr` (default `127.0.0.1:9100`; `disabled` to skip). 13 pre-registered metrics: `oxide_raft_term`, `oxide_raft_commit_index`, `oxide_raft_last_applied`, `oxide_raft_log_length`, `oxide_raft_role`, `oxide_raft_snapshot_age_seconds`, `oxide_raft_snapshot_bytes`, `oxide_peer_match_index{peer=...}`, `oxide_peer_next_index{peer=...}`, `oxide_tx_pending_count`, `oxide_tx_timeout_aborted_total`, `oxide_tx_admin_aborted_total`, plus a `/health` endpoint. OpenTelemetry is wired as a `tracing` no-op (recommended Rust idiom; the actual OTel exporter is deferred to ops-time so operators don't pay for an OTel collector they don't run). 13 new lib unit tests (registry × 5 + server × 5 + tracer × 3) + 4 new integration tests in `tests/metrics_endpoint.rs`. **255 → 268 lib tests, +4 integration tests.** |
| ✅ #9 (merged via PR #43) | `test(deploy): 3-node cross-process smoke test under CI` | `tests/cross_process_smoke.rs` boots 3 real `oxide-kv` processes on TCP ports 9001/9002/9003 (Raft RPC) + 9101/9102/9103 (JSON client) + 9100/9200/9300 (Prometheus `/metrics`) via `deploy/scripts/bootstrap-cluster.sh`. Verifies the deploy path (3 active tests: metrics endpoint responds on all 3 ports, JSON client protocol works on every node, election machinery runs within 10s). 3 consensus-scope tests were initially `#[ignore]`-gated; CI step in `.github/workflows/rust.yml` runs after `cargo build --release --bin oxide-kv` with `--test-threads=1`. **Bugs surfaced, since fixed**: (a) pre-vote tie / split-brain → fixed by PR #51 (pre-vote must not self-vote; incumbent leader must not demote on same-term AE); (b) multi-node `commit_index` stuck at 0 → fixed by PR #50 (bootstrap peer-list off-by-one); (c) leader churn from pre-vote timeout too close to heartbeat → resolved by the same fixes. All 3 gated tests verified passing on master (2026-08-07) and un-`#[ignore]`d — they now run in every CI smoke run. |
| ✅ #10 (merged via PR #50) | `fix(deploy): bootstrap-cluster.sh peer-list off-by-one` | The bootstrap script generated an off-by-one peer list that broke membership + multi-node replication (`commit_index` stuck at 0). Discovered by the PR #9 smoke test. |
| ✅ #11 (merged via PR #51) | `fix(raft): pre-vote must not self-vote; incumbent leader must not demote on same-term AE` | Root causes of the split-brain surfaced by the PR #9 smoke test: pre-vote counted a self-vote, and an incumbent leader demoted itself on receiving a same-term AppendEntries from a rival. |
...
2. **Joint consensus** lets us add / remove a node without restarting the cluster; the cluster continues to serve ops during the transition. *(PR #6 implemented: dual-majority `maybe_commit`, leader intercepts `AddNode`/`RemoveNode` and writes `InstallConfiguration` log entry, two-phase Joint→Simple transition. PR #6a implemented: cold-new-server catch-up via `JoinCluster` RPC so a brand-new server can join without being preconfigured by an operator.)*
...
All six acceptance criteria verified on `master`:

1. ✅ Pre-vote merged (PR #37) + split-brain hotfix (PR #51); `pre_vote_recovery` integration test green on master.
2. ✅ Joint consensus (PR #38) + JoinCluster cold-join (PR #39) merged.
