...
| P8 | Pre-vote, joint consensus, tx timeout, metrics, prod cluster test | 🔄 Active | PR #33-#36 merged; PR #37 (pre-vote), PR #38 (joint consensus), PR #39 (JoinCluster) merged; PR #7 (tx timeout + admin abort) in flight | Closing the disruptive-server problem + ops hardening |
...
| 🔵 #7 (this PR, OPEN) | `feat(raft): tx timeout + admin-driven abort` | Coordinator-side timeout sweep (`run_tx_timeout_loop` spawned in `main.rs`, `OXIDE_TX_TIMEOUT_MS` default 30 s, sweep interval default 1 s) + admin `AbortTx` JSON RPC. Both translate to a `DecideTx(Abort)` log entry which replicates through the normal AppendEntries path; followers apply via existing `apply_logs` and purge `pending_txs`. New `PendingTx::begin_unix_ms` field with `serde(default)` for forward-compat. Wire: `Command.Body::abort_tx = 11`. 6 lib tests (`raft/node.rs` + `raft/coordinator.rs`) + 4 state-machine tests + 3 integration tests in `tests/tx_timeout_abort.rs` = **249 → 255 lib tests, +3 integration tests.** Priority-B recovery (participant-side autonomous abort) deferred. |
