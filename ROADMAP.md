...
| P8 | Pre-vote, joint consensus, tx timeout, metrics, prod cluster test | 🔄 Active | PR #33-#42 merged; PR #9 (3-node smoke test) in review; PR #10+ (consensus bugs surfaced by #9) next | Closing the disruptive-server problem + ops hardening |
...
1. **Virtual clock + virtual network.** Replace wall-clock timers and
   ...
   (`crates/oxide-kv/src/raft/transport.rs`) and the clock behind traits, with a
